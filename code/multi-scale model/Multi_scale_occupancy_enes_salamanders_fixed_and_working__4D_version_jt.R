...
#we formatting the data in script to see if it helps spot issue 
  ...
  year.sites= list(9)
  for (t in 1:n.years){
    I[t] = length(enes$site_id[enes$year==years[t]])
...
for(chain in 1:n.chains){
  ...
  Niminits <- list(beta0.psi = 0, beta0.theta = 0, alpha0 = 0,
                  ...
                  sd.psi.year = runif(1, 0.1, 1), sd.theta.year = runif(1, 0.1, 1), sd.p.year = runif(1, 0.1, 1), z = z_init, w = w_init)
  
  # parameters to monitor
  parameters <- c("beta0.psi.year", "beta0.theta.year", "alpha0.year", "beta0.psi", "beta1.psi.BU", "beta2.psi.HB", "beta3.psi.HU", "beta4.psi.BS")
 
  str(K2D)
  str(y.4D)

  # Multi-scale occupancy model for estimating the occupancy state of use of plot j, given the occupancy status of site i from k surveys across t years
  NimModel <- nimbleCode({
    ...
    for(t in 1:nyears) {
      ...
      # estimate theta (plot usage) as function of covs (no covs currently)
      for (i in 1:I[t]){
        for(j in 1:J[i]){
        logit(theta[j,i,t]) <- beta0.theta.year[t] 
  ...
  conf <- configureMCMC(Rmodel, monitors=parameters, thin=1, useConjugacy=FALSE) # this sets thinning interval to every 2nd value
  ...
  Cmcmc$run(25000,reset=FALSE) #Can keep extending the run by rerunning this line  #this sets n.iterations
  end.time <- Sys.time()
...
cor <- cor(a[, c("alpha0.year[1]", "alpha0.year[2]", "alpha0.year[3]", "alpha0.year[4]", "alpha0.year[5]", "alpha0.year[6]",     
                          "alpha0.year[7]",      "alpha0.year[8]" ,     "alpha0.year[9]",      "beta0.psi",           "beta0.psi.year[1]" ,  "beta0.psi.year[2]",  
"beta0.psi.year[3]",   "beta0.psi.year[4]",   "beta0.psi.year[5]",   "beta0.psi.year[6]",   "beta0.psi.year[7]",   "beta0.psi.year[8]" , 
"beta0.psi.year[9]",   "beta0.theta.year[1]", "beta0.theta.year[2]", "beta0.theta.year[3]", "beta0.theta.year[4]", "beta0.theta.year[5]",
"beta0.theta.year[6]", "beta0.theta.year[7]", "beta0.theta.year[8]", "beta0.theta.year[9]", "beta1.psi.BU",        "beta2.psi.HB",  "beta3.psi.HU",
"beta4.psi.BS" )])
