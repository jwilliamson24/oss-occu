...
for(chain in 1:n.chains){
  ...
  parameters <- c("beta0.psi.year", "beta0.theta.year", "alpha0.year", "beta0.psi", "beta0.theta", "alpha0", 
  ...
  conf <- configureMCMC(Rmodel, monitors=parameters, thin=7, useConjugacy=FALSE) # this sets thinning interval to every 2nd value
  
  # custom RW updates - block parameters which display high posterior correlation
  # try the 

    conf$addSampler(target = c("beta0.psi", "sd.psi.year"),
  ...
  Cmcmc$run(350000,reset=FALSE) #Can keep extending the run by rerunning this line  #this sets n.iterations
  end.time <- Sys.time()
...
cor <- cor(a[, c("alpha0",  "beta0.psi","beta0.theta", "beta1.psi.BU", "beta2.psi.HB",  "beta3.psi.HU",
"beta4.psi.BS" )])
